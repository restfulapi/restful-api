...
* HTTP PATCH support including the ability to apply a JSON Patch to a resource and delegate to a service 'update' method, or alternatively to simply use a service 'patch' method
* Consistent use of HTTP status codes and headers, including caching headers,  CORS headers, and the Link header for pagination
...
**Important:** parseRequest must be set to false; otherwise, grails will attempt to parse the request body and add values from it to the params object.  The restful plugin is designed to pass any content from the body to the service layer via an extracted map, not the params object.
    ...
    static mappings = {
        ...
        //
        "/api/$pluralizedResourceName/$id"(controller:'restfulApi') {
            action = [GET: "show", PUT: "update", PATCH: "patch",
                      DELETE: "delete"]
            parseRequest = false
            constraints {
                // to constrain the id to numeric, uncomment the following:
...
The patch method is passed the a content Map extracted from the request, and the Grails params Map.  (The 'content' map is generated by the Extractor registered for the resource and format.)

The patch method is responsible for using the map to patch an instance of the resource.  The id of the resource will be contained in the params Map, per the UrlMappings.

NOTE: The plugin can alternatively be configured to apply a [JSON Patch](https://tools.ietf.org/html/rfc6902), and then delegate to a service 'update' method using an already-patched resource. 

Regardless of whether you implement patch within your service or rely on the controller to apply a JSON Patch, PATCH support is not automatic and must be configured.  The followign configuration shows how to support PATCH using a service 'patch' method.
...
## <a id="json-patch"></a>JSON Patch

HTTP PATCH is preferred over HTTP PUT for making changes to an existing resource.  HTTP PUT is intended to 'replace' a resource, although as many APIs do not yet support PATCH it is common for PUT support to implement 'PATCH' semantics (to partially modify an existing resource).

The controller may be configured to delegate PATCH requests to a service 'patch' method. In addition, the controller can apply a JSON Patch itself and delegate to a service 'update' method (using an already-patched resource).

The following configures the controller to apply a [JSON Patch](https://tools.ietf.org/html/rfc6902) to an existing resource before delegating to a service 'update' method. Note the representation is 'application/json-patch+json', which is the standard media type for JSON Patch regardless of the resource being patched. Also note no marshaller is configured, as the response to a PATCH is the result of applying the patch (i.e., an updated resource using the representation requested via the Accept header).  Lastly, note the use of a specific extractor that is provided, which is able to extract a JSON Array versus a JSON Object that would be used to support a PUT request. 
